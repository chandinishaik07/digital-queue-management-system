...
export default function AgentDashboard({ user }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const qRes = await fetch('https://queuepro-backend-2pl7.onrender.com/api/queue')
      const q    = await qRes.json()
      setQueue(Array.isArray(q) ? q : [])

      // Fetch all counters to find which one is assigned to this agent
      const cRes     = await fetch('https://queuepro-backend-2pl7.onrender.com/api/counters')
  ...
  const markComplete = async () => {
    ...
    try {
      await fetch(`https://queuepro-backend-2pl7.onrender.com/api/queue/${servingToken._id}`, {
        method:  'PUT',
  ...
  const toggleStatus = async () => {
    ...
    try {
      await fetch(`https://queuepro-backend-2pl7.onrender.com/api/counters/${myCounter._id}/status`, {
        method:  'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const assignAppointment = async (tokenId, tokenNumber) => {
    ...
    try {
      ...
      const res = await fetch(
        `https://queuepro-backend-2pl7.onrender.com/api/queue/${tokenId}/appointment`,
        {
          method:  'PUT',
          headers: { 'Content-Type': 'application/json' },
          body:    JSON.stringify({
